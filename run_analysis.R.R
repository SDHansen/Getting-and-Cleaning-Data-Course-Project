...
##  It assumes that you have already downloaded and unzipped the data into sub-directory called data
...
## Set project directory.  Can be commented out or updated to reflect location of files
setwd("F:\\Data Scientist\\Data Science Track\\Getting and Cleaning Data\\ClassProject")
...
subject<-read.table("./data/test/subject_test.txt", col.names="subject")
activity_id<-read.table("./data/test/y_test.txt", col.names="activity_id")
test_data<-cbind(subject, activity_id, X_test)

## Load Train data

X_train<-read.table("./data/train/X_train.txt", col.names = features)
subject<-read.table("./data/train/subject_train.txt", col.names="subject")
activity_id<-read.table("./data/train/y_train.txt", col.names="activity_id")
